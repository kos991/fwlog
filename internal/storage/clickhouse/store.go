...
func (s *ClickHouseStore) Ready() bool {
	return s != nil && s.conn != nil
}

func (s *ClickHouseStore) PrepareBatch(ctx context.Context, query string, opts ...driver.PrepareBatchOption) (driver.Batch, error) {
	if s == nil || s.conn == nil {
		return nil, fmt.Errorf("clickhouse connection is not initialized")
	}
	return s.conn.PrepareBatch(ctx, query, opts...)
}
