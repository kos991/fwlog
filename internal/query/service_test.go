...
func TestBuildVisibleRangesQueriesReadyAndPartialImportingDates(t *testing.T) {
	...
	if visibility.Message != partialVisibilityMessage {
	...
	if got := visibility.SkippedDates[0]; !got.LogDate.Equal(dateOnly(2026, 6, 29)) || got.Reason != missingStateReason {
		t.Fatalf("missing-state skipped date = %#v", got)
	}
	if got := visibility.SkippedDates[1]; !got.LogDate.Equal(dateOnly(2026, 7, 1)) || got.Status != StatusPending || got.Reason != notReadyReason {
...
func TestBuildVisibleRangesSkipsNonQueryableStatuses(t *testing.T) {
	...
	for i, want := range []struct {
		...
		{dateOnly(2026, 6, 28), StatusIdle, notQueryableReason},
		{dateOnly(2026, 6, 29), StatusScanning, notQueryableReason},
...
func TestVisibilityTextIsUTF8Chinese(t *testing.T) {
	...
	if len(visibility.SkippedDates) != 1 || visibility.SkippedDates[0].Reason != notReadyReason {
...
func TestQueryResponseJSONFields(t *testing.T) {
	resp := QueryResponse{
		...
		Visibility: QueryVisibility{
			Partial: true,
			Message: partialVisibilityMessage,
		},
	}
