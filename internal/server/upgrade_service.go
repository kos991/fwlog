...
import (
	...
	"fwlog/internal/upgrade"
)

const (
	githubRepoOwner       = "kos991"
	githubRepoName        = "fwlog"
	linuxUpgradeAssetName = "fwlog_linux_amd64"
	installedBinaryPath   = "/opt/fwlog/fwlog"
)
...
var upgradeBackupDir = "/data/fwlog/backups"
var osReleasePath = "/etc/os-release"
var upgradeTempRoot = "/opt/fwlog/tmp"
var upgradeHTTPClient = &http.Client{Timeout: upgradeHTTPTimeout}
var runCommand = func(ctx context.Context, name string, args ...string) ([]byte, error) {
	return exec.CommandContext(ctx, name, args...).CombinedOutput()
}

type UpgradeState = upgrade.State
type UpgradeStatus = upgrade.Status

const (
	UpgradeStateIdle      = upgrade.StateIdle
	UpgradeStateRunning   = upgrade.StateRunning
	UpgradeStateSucceeded = upgrade.StateSucceeded
	UpgradeStateFailed    = upgrade.StateFailed
)
...
func runPackageManagerCommand(ctx context.Context, name string, args ...string) ([]byte, error) {
	if _, err := lookPath("systemd-run"); err == nil {
		systemdArgs := []string{
			...
			"--property=ReadWritePaths=/ /var/lib/dpkg /var/lib/rpm /opt/fwlog /data",
			name,
		}
...
func backupCurrentInstall(ctx context.Context) (string, error) {
	...
	for _, item := range []struct {
		...
		{"/etc/systemd/system/fwlog.service", "fwlog.service"},
		{"/opt/fwlog/VERSION", "VERSION"},
	} {
