...
func TestReleaseHasRequiredLinuxUpgradeAssets(t *testing.T) {
	release := githubRelease{
		TagName: "v2.1.0",
		HTMLURL: "https://github.com/kos991/fwlog/releases/tag/v2.1.0",
		Assets: []githubReleaseAsset{
			{Name: linuxUpgradeAssetName, BrowserDownloadURL: "https://example.test/legacy"},
			{Name: "fwlog-upgrade-v2.1.0.x86_64.rpm", BrowserDownloadURL: "https://example.test/upgrade.rpm"},
			{Name: "fwlog-upgrade_2.1.0_amd64.deb", BrowserDownloadURL: "https://example.test/upgrade.deb"},
		},
	}
...
func TestReleaseReportsMissingUpgradeAssets(t *testing.T) {
	...
	want := []string{"fwlog-upgrade-v2.1.0.x86_64.rpm", "fwlog-upgrade_2.1.0_amd64.deb"}
...
func TestInstallUpgradePackageUsesPackageManager(t *testing.T) {
	calls := stubCommandRunner(t, func(ctx context.Context, name string, args ...string) ([]byte, error) {
		...
		if args[0] != "--wait" || args[1] != "--collect" || args[2] != "--pipe" || args[3] != "--service-type=oneshot" || args[4] != "--property=ProtectSystem=off" || args[5] != "--property=ProtectHome=off" || args[6] != "--property=PrivateTmp=no" || args[7] != "--property=ReadWritePaths=/ /var/lib/dpkg /var/lib/rpm /opt/fwlog /data" || args[8] != "dpkg" || args[9] != "-i" || args[10] != "/tmp/fwlog-upgrade.deb" {
			t.Fatalf("unexpected systemd-run args: %#v", args)
		}
...
func TestExecuteSystemUpgradeInstallsUpgradePackage(t *testing.T) {
	...
	defer restoreOS()
	restoreHTTP := stubHTTPClient(t, map[string]string{
		"https://api.github.com/repos/kos991/fwlog/releases/tags/v2.1.0": `{
			"tag_name":"v2.1.0",
			"html_url":"https://github.com/kos991/fwlog/releases/tag/v2.1.0",
			"assets":[
				{"name":"fwlog_linux_amd64","browser_download_url":"https://downloads.test/fwlog_linux_amd64"},
	...
	if err := executeSystemUpgrade(context.Background(), upgradeTarget{Version: "v2.1.0"}, &status); err != nil {
	...
	if !strings.HasPrefix(commands[0], "dpkg-deb --contents ") || !strings.Contains(commands[1], "systemd-run --wait --collect --pipe --service-type=oneshot --property=ProtectSystem=off --property=ProtectHome=off --property=PrivateTmp=no --property=ReadWritePaths=/ /var/lib/dpkg /var/lib/rpm /opt/fwlog /data dpkg -i ") {
...
func TestExecuteSystemUpgradeVerifiesChecksumUsingReleaseAssetName(t *testing.T) {
	...
	restoreHTTP := stubHTTPClient(t, map[string]string{
		"https://api.github.com/repos/kos991/fwlog/releases/tags/v2.0.15": `{
			"tag_name":"v2.0.15",
			"html_url":"https://github.com/kos991/fwlog/releases/tag/v2.0.15",
			"assets":[
				{"name":"fwlog_linux_amd64","browser_download_url":"https://downloads.test/fwlog_linux_amd64"},
				{"name":"fwlog-upgrade-v2.0.15.x86_64.rpm","browser_download_url":"https://downloads.test/fwlog-upgrade.rpm"},
				{"name":"fwlog-upgrade_2.0.15_amd64.deb","browser_download_url":"https://downloads.test/fwlog-upgrade.deb"},
		...
		"https://downloads.test/checksums.txt":     packageHash + "  /home/runner/work/fwlog/fwlog/release/fwlog-upgrade_2.0.15_amd64.deb\n",
	})
	defer restoreHTTP()
	originalUpgradeTempRoot := upgradeTempRoot
	upgradeTempRoot = t.TempDir()
...
func TestReplaceFileAtomicSwapsBinaryContent(t *testing.T) {
	...
	targetPath := filepath.Join(dir, "fwlog")
...
func TestUpgradeAPIsRequireSession(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{method: http.MethodPost, path: "/api/upgrade/run", body: `{"version":"v2.1.0"}`},
	} {
...
func TestUpgradeRunRejectsConcurrentTask(t *testing.T) {
	...
	firstReq := httptest.NewRequest(http.MethodPost, "/api/upgrade/run", bytes.NewBufferString(`{"version":"v2.1.0"}`))
	...
	secondReq := httptest.NewRequest(http.MethodPost, "/api/upgrade/run", bytes.NewBufferString(`{"version":"v2.1.0"}`))
...
func TestDecodeReleaseManifestReadsRuntimeRequirement(t *testing.T) {
	manifest, err := decodeReleaseManifest(strings.NewReader(`{"app_version":"v2.0.0","runtime_version":"clickhouse-25.8.27.1"}`))
	if err != nil {
		t.Fatal(err)
	}
...
func TestUpgradeRunIgnoresRequestedVersionAndTargetsLatestRelease(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/upgrade/run", bytes.NewBufferString(`{"version":"v2.0.1"}`))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(cookie)
	res := httptest.NewRecorder()
	router.ServeHTTP(res, req)
