...
func TestSystemdServiceAllowsRuntimeDataDirectory(t *testing.T) {
	content := []byte(readRepoFile(t, "fwlog.service"))
	...
	if !contains(paths, "/opt/fwlog") {
		t.Fatalf("ReadWritePaths must include /opt/fwlog so the service can replace its binary during Release upgrades; got %q", match[1])
	}
}

func TestSystemdServiceDefaultsAutoScanOff(t *testing.T) {
	content := readRepoFile(t, "fwlog.service")

	if !strings.Contains(content, `Environment="AUTO_SCAN_ENABLED=false"`) {
		t.Fatal("fwlog.service must explicitly default AUTO_SCAN_ENABLED to false for new installs")
	}
}
...
func TestCIWorkflowMatchesClickHouseBuildFlow(t *testing.T) {
	assertWorkflowMatchesBuildFlow(
		...
		[]string{
			...
			"go build -trimpath -ldflags \"-s -w\" -o dist/fwlog_linux_amd64 ./cmd/fwlog",
			"PACKAGING_MODE=full bash packaging/build-server-packages.sh --version 0.0.0 --binary dist/fwlog_linux_amd64 --output dist",
			"PACKAGING_MODE=upgrade bash packaging/build-server-packages.sh --version 0.0.0 --binary dist/fwlog_linux_amd64 --output dist",
...
func TestReleaseWorkflowMatchesClickHouseBuildFlow(t *testing.T) {
	assertWorkflowMatchesBuildFlow(
		...
		[]string{
			...
			"npm test",
			"go test -race -count=1 ./...",
			"go build -trimpath -ldflags \"-s -w -X fwlog/internal/server.appVersion=$version\" -o \"release/$asset\" ./cmd/fwlog",
			"GOOS: linux",
			"GOARCH: amd64",
			"-X fwlog/internal/server.appVersion",
			"sudo apt-get install -y cpio rpm",
			...
			"cp \"release/fwlog-upgrade-v${pkg_version}.x86_64.rpm\" \"release/fwlog_kylin-server_amd64.rpm\"",
			"cp \"release/fwlog-upgrade_${pkg_version}_amd64.deb\" \"release/fwlog_debian-server_amd64.deb\"",
			"fwlog_linux_amd64",
			"fwlog_kylin-server_amd64.rpm",
			"fwlog_debian-server_amd64.deb",
...
func TestServerPackageAssetsAreRequiredForUpgradeChecks(t *testing.T) {
	release := githubRelease{
		...
		Assets: []githubReleaseAsset{
			...
			{Name: "fwlog-upgrade-v2.1.0.x86_64.rpm", BrowserDownloadURL: "https://example.test/rpm"},
			{Name: "fwlog-upgrade_2.1.0_amd64.deb", BrowserDownloadURL: "https://example.test/deb"},
		},
	}
