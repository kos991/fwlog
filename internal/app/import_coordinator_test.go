...
func TestNewAppUsesConfiguredWorkerLimitForSourceImports(t *testing.T) {
	app := NewApp(Config{Workers: 3})
	if cap(app.imports.sourceSem) != 3 {
		t.Fatalf("source import concurrency = %d, want 3", cap(app.imports.sourceSem))
	}
}
