...
func TestTask1FilesNoLongerReferenceDuckDB(t *testing.T) {
	...
	assertFileExcludes(t, filepath.Join("cmd", "nat-query-service", "main.go"), "duckdb", "go-duckdb", "database/sql")
	assertFileExcludes(t, "nat-query-service.service", "DB_FILE=", "nat_logs.duckdb")
	assertFileIncludes(t, "nat-query-service.service",
		`Environment="LOG_TAG=`,
